"""Programmatic DOCX report generation without a template file.

Generates the Chongqing inspection report (重庆G210交安设施检测报告.docx)
from scratch with python-docx: headings, body text, tables, captions,
matplotlib images, and section numbering. Used when the original Word
...
def make_report(config, segments, height_stats, height_records, bolt_stats, bolt_records, disease_image_index, temp_dir, log=lambda _x: None):
...
def run(config, segments, height_stats, height_records, bolt_stats, bolt_records, disease_image_index, log=lambda _x: None):
    import tempfile

    with tempfile.TemporaryDirectory(prefix="g210_report_builtin_") as temp_dir:
        config.output_dir.mkdir(parents=True, exist_ok=True)
        log("未检测到内置 Word 模板，切换到程序化报告生成模式。")
        return make_report(config, segments, height_stats, height_records, bolt_stats, bolt_records, disease_image_index, temp_dir, log)
